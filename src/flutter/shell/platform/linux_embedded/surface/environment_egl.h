 ...
 public:
  EnvironmentEgl(EGLenum platform, EGLNativeDisplayType platform_display,
    ...
    if (platform) {
        ...
        if (GetPlatformDisplay) // FIXME: maybe not null even if not found w/o KHR_client_get_all_proc_addresses
            display_ = GetPlatformDisplay(platform, platform_display, nullptr);
    }
